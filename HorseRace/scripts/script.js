...
function updateCurrentRace(race) {
    ...
    for(let i = 0; i < race.horses.length; i++) {
        let row = `<tr><td style="background-color: ${colors[i]}">${i+1}</td><td>${race.horses[i].name}</td><td>${race.horses[i].getOdds(race.houseCut)}</td><td>${formatOdds(race.horses[i].getOdds(race.houseCut))}</td><td>${race.horses[i].raceSum} - ${race.horses[i].place}</td></tr>`
...
function updateNextRace(race) {
    ...
    if(race != null) {
        for(let i = 0; i < race.horses.length; i++) {
            let row = `<tr><td style="background-color: ${colors[i]}">${i+1}</td><td>${race.horses[i].name}</td><td>${race.horses[i].getOdds(race.houseCut)}</td><td>${formatOdds(race.horses[i].getOdds(race.houseCut))}</td><td>${race.horses[i].calculatedProb}</td></tr>`
...
function initSettings() {
    ...
    for(let i = 0; i < settings.horseTypes.length; i++) {
        let item = `<label for='${settings.horseTypes[i].ref}'>${data[settings.horseTypes[i].ref].name}</label><input type='checkbox' id='${settings.horseTypes[i].ref}' ${settings.horseTypes[i].active ? 'checked' : ''}/>`
